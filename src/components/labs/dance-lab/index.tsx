...
export function DanceLab({ avatar, onBack }: DanceLabProps) {
  ...
  // Shape library is read-only here — used for cross-library name
  // collision checks so a move can't claim a name a shape already owns.
  ...
  const sequencer = useMoveSequencer({
    ...
    // Dance lab only routes Move tokens through the sequencer — preview
    // and play paths never construct spawn tokens. The callback is
    // required by SequencerDeps so misconfiguration in any consumer
    // surfaces at compile time; this no-op is the documented dance-lab
    // posture.
    onSpawnSticker: () => {},
  });
  ...
  // Check the draft's names against both libraries (moves AND shapes
  // share one flat command namespace). excludeMoveId skips the move being
  // updated (keeping your own name isn't a collision); pass null for
  // save-as-new, where colliding with the source move's name forces the
  // user to pick a fresh one. excludeShapeId is always null here — the
  // dance lab never edits a shape, so no shape can be self-exempt.
  const checkNames = (excludeMoveId: string | null): NameErrors => ({
    primary: isCommandTaken(
      draft.primary,
      library,
      shapeLibrary,
      excludeMoveId,
  ...
  const handlePlayMove = (move: Move) => {
    sequencer.runChain([{ type: "move", move }]);
  };

  return (
    <div className="lab lab--dance dance-lab">
      <header className="lab__header">
        ...
        <span className="lab__title">Dance Lab</span>
      </header>
      ...
      <main className="lab__main">
        ...
        <MoveForm
          ...
          onPrimaryChange={(primary) => {
            setDraft({ ...draft, primary: filterCommandName(primary) });
            setNameErrors((e) => ({ ...e, primary: false }));
          }}
          onSecondaryChange={(secondary) => {
            setDraft({ ...draft, secondary: filterCommandName(secondary) });
        ...
        <footer className="lab__footer">
          <a
            ...
            href="/documentation.html#dance-lab"
            target="_blank"
            rel="noopener noreferrer"
          >
            Read more about the Dance Lab →
          </a>
        </footer>
      </main>
    </div>
